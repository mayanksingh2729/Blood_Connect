...
router.put("/donations", auth, async (req, res) => {
    try {
        const donation = await Donations.updateOne({ _id: req.body.id }, { status: req.body.status });
        if (donation.matchedCount === 0) {
...
router.put("/requests", auth, async (req, res) => {
    try {
        const request = await Requests.updateOne({ _id: req.body.id }, { status: req.body.status });
...
router.put("/", auth, async (req, res) => {
    try {
        const update = await BloodBank.updateOne({ _id: req.user }, req.body);
